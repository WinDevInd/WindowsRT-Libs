    ...
    internal class HtmlParser
    {
        ...
        private XElement ParseHtmlContent()
        {
            ...
            if (htmlRootElement.FirstNode is XElement &&
                htmlRootElement.FirstNode == htmlRootElement.LastNode &&
                ((XElement)htmlRootElement.FirstNode).Name.LocalName.ToString().ToLower() == "html")
            {
        ...
        {
            XElement htmlElementCopy = new XElement(htmlElement.Name);
            foreach (var attr in htmlElement.Attributes())
            {
                
                htmlElementCopy.Add(new XAttribute(attr.Name, attr.Value));
            }
        ...
        private void OpenStructuringElement(XElement htmlElement)
        {
            ...
            if (HtmlSchema.IsBlockElement(htmlElement.Name.LocalName.ToLower()))
            {
                while (_openedElements.Count > 0 && HtmlSchema.IsInlineElement(_openedElements.Peek().Name.LocalName.ToLower()))
                {
            ...
            if (_openedElements.Count > 0)
            {
                ...
                if (HtmlSchema.ClosesOnNextElementStart(htmlParent.Name.LocalName.ToLower(), htmlElement.Name.LocalName.ToLower()))
                {
                    _openedElements.Pop();
        ...
        private bool IsElementOpened(string htmlElementName)
        {
            ...
            {
                if (openedElement.Name.LocalName.ToLower() == htmlElementName)
                {
                    return true;
                }
            }
        ...
        private void CloseElement(string htmlElementName)
        {
            ...
            if (_pendingInlineElements.Count > 0 && _pendingInlineElements.Peek().Name.LocalName.ToLower() == htmlElementName)
            {
            ...
            else if (IsElementOpened(htmlElementName))
            {
                while (_openedElements.Count > 1) // we never pop the last element - the artificial root
                {
                    ...
                    if (htmlOpenedElement.Name.LocalName.ToLower() == htmlElementName)
                    {
                        return;
                    }

                    if (HtmlSchema.IsInlineElement(htmlOpenedElement.Name.LocalName.ToLower()))
                    {
